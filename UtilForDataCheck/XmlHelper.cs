...
namespace UtilForDataCheck
{
    ...
    public class XmlHelper
    {
        ...
        //判断节点是否存在
        public bool IsNodeExist(string xmlpath, string xpath)
        {
            bool flag = true;

            XmlNode node = GetXmlNodeByXpath(xmlpath, xpath);
            if (node == null)
            {
                flag = false;
            }
        ...
        /// <param name="nodeXPath">待更新节点的XPath</param>
        /// <param name="htAttr">属性表</param>
        /// <returns></returns>
        public bool UpdateNode(string xmlpath, string nodeXPath, Hashtable htAttr)
        {
            try
            {
                xmldoc = new XmlDocument();
                ...
                xmlelem = (XmlElement)node;
                if (xmlelem.HasAttributes)//如果节点如属性，则更改它的属性
                {
                    foreach (DictionaryEntry de in htAttr)//遍历属性哈希表
                    {
        ...
        /// <param name="_xpath">Xpath</param>
        /// <param name="_attrname">属性名</param>
        /// <returns></returns>
        public bool IsAttrExist(string _xmlpath,string _xpath,string _attrname)
        {
            if (string.IsNullOrEmpty(_xmlpath) || string.IsNullOrEmpty(_xpath) || string.IsNullOrEmpty(_attrname))
            {
                return false;
            }

            XmlNode node = GetXmlNodeByXpath(_xmlpath, _xpath);

            return IsAttrExist(node, _attrname);
        }

        /// <summary>
        /// 判断节点是否存在指定属性
        /// </summary>
        /// <param name="_node">Xml节点</param>
        /// <param name="_attrname">属性名</param>
        /// <returns></returns>
        public bool IsAttrExist(XmlNode _node, string _attrname)
        {
            if (_node == null || string.IsNullOrEmpty(_attrname))
            {
                return false;
            }
